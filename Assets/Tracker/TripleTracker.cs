...
public class TripleTracker : MonoBehaviour
{
    ...
    [Tooltip("How often to sample the tracker data in seconds (e.g., 0.2f = 5 times per second)")]
    ...
    private void Start()
    {
       ...
       // Trigger now acts as a toggle button to start/stop the automatic scanning routine
       _controllerActions.Trigger.started += HandleTriggerToggle;
       _controllerActions.Bumper.canceled += HandleOnBumperRelease;

        trackedObject.SetActive(true);
        trackedObject.transform.GetChild(4).gameObject.GetComponent<TextMeshPro>().text = "Press Trigger to toggle automatic scanning mode.";
    ...
    // Keep this lightweight: just store the latest marker data as it streams in
    ...
    // This loop executes automatically at your exact defined rate interval
    private IEnumerator AutomatedScanRoutine()
    {
        while (_isScanningAutomated)
        {
            // Only process if we have actually captured a frame since our last wait check
            if (_hasFreshData)
            {
                Debug.Log($"[Auto-Scan] Processing frame for marker {aruco_id} at set rate interval.");

                // 1. Queue management for moving window
                ...
                // 2. Mathematical rolling average calculation
            ...
            // Yield control and wait for your specified time delay interval
            yield return new WaitForSeconds(scanInterval);
        }
    }

    // Toggles the automated coroutine scanning loop on or off with each single trigger click
    private void HandleTriggerToggle(InputAction.CallbackContext obj)
    {
        _isScanningAutomated = !_isScanningAutomated;
        ...
        if (_isScanningAutomated)
        {
            ...
            // Start the infinite sampling clock
        ...
        else
        {
            ...
            ExecuteOriginSubmit(); // Fire off your network/Noodles calls automatically when turned off
        }
    }
    ...
    private void Update()
    {
        if (_controllerActions.Bumper.IsPressed())
        {
            ...
            try
            {
                ...
                // OLD POINT-BASED POSTION UPDATED INVOKE CODE
...
//                 // 1. Add the newest position to our rolling window
//                 positionBuffer.Enqueue(data.Pose.position);

//                 // 2. If the buffer is full, drop the oldest scan frame
//                 if (positionBuffer.Count > windowSize)
//                 {
//                     positionBuffer.Dequeue();
//                 }

//                 // 3. Sum up all positions currently inside our window
//                 Vector3 sum = Vector3.zero;
//                 foreach (Vector3 pos in positionBuffer)
//                 {
//                     sum += pos;
...
//                 // 4. Set position to the exact mathematical average of the window
//                 this.transform.position = sum / positionBuffer.Count;

//                 // 5. Update rotation (Slerp is still recommended here to prevent rotation flipping)
//                 Quaternion targetRotation = data.Pose.rotation * Quaternion.Euler(-90, 0, 180);
//                 this.transform.rotation = Quaternion.Slerp(this.transform.rotation, targetRotation, 0.2f);

//                 // 6. Sync to your tracked object
//                 trackedObject.transform.GetChild(4).gameObject.SetActive(true);
//                 trackedObject.transform.position = this.transform.position;
//                 trackedObject.transform.rotation = this.transform.rotation;
//             }
